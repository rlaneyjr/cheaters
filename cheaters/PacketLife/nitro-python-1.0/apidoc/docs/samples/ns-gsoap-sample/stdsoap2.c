...
#endif
#ifdef SOAP_DEBUG
  soap_set_test_logfile(soap, "/var/log/TEST.log");
  soap_set_sent_logfile(soap, "/var/log/SENT.log");
  soap_set_recv_logfile(soap, "/var/log/RECV.log");
  ...
  { if (soap->other)
    ...
    { soap->peeked = 0;
	  ...
	  if (soap->body && !*soap->href) {
          ...
          soap_id_forward( soap, soap->href, tag, 0, 0, 0, sizeof(int), 0, NULL);
	  }
        ...
        { s = soap_strrchr(tp->value, '[');
          ...
          soap->arrayType[sizeof(soap->arrayType)-1] = '\0';
        ...
        else if (!soap_match_tag(soap, tp->name, "SOAP-ENC:offset"))
			{
				...
				soap->arrayOffset[sizeof(soap->arrayOffset)-1] = '\0';
			}
...
#ifndef NETSCALER
	// 2004oct21/reilbert - clients do not normally send domain and path
	size_t n;
	if (!domain)
...
#ifndef NETSCALER
	// 2004oct21/reilbert - clear the named cookie without regard to domain or path
	if (!domain)
		domain = soap->cookie_domain;
	if (!domain)
...
#ifndef NETSCALER
		&& !strcmp(q->domain, domain) && !strncmp(q->path, path, strlen(q->path))
#endif
		)
    { if (q->value)
        SOAP_FREE(soap, q->value);
	  ...
	  /* Fix for BUG64110: Config Infra: Memory not freed messages on using API's.
	   * q->name was allocated memory in soap_set_cookie() so freed it here.
	   */
	  if (q->name)
		  SOAP_FREE(soap, q->name);
      *p = q->next;
